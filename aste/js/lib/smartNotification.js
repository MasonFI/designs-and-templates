...
(function ($) {
    $.SmartMessageBox = function (settings, callback) {
        ...
        if (settings.input != undefined) {
            ...
            case "text":
                InputType = "<input class='form-control' type='" + settings.input + "' id='txt" +
                	SmartMSGboxCount + "' placeholder='" + settings.placeholder + "' value='" + settings.inputValue + "'/>";

                break;
            case "password":
                InputType = "<input class='form-control' type='" + settings.input + "' id='txt" +
                    SmartMSGboxCount + "' placeholder='" + settings.placeholder + "'/>";
                break;
        ...
        // MYDIGI support two possible inputs
        var InputType2 = "";
        if (settings.secondInput != undefined) {
            HasInput = 1;
            settings.secondInput = settings.secondInput.toLowerCase();
			settings.secondInputValue = settings.secondInputValue ? settings.secondInputValue.replace(/'/g, "&#x27;") : ''; // new code

            switch (settings.secondInput) {
            case "text":
                InputType2 = "<input class='form-control' type='" + settings.secondInput + "' id='sndTxt" +
                	SmartMSGboxCount + "' placeholder='" + settings.secondPlaceholder + "' value='" + settings.secondInputValue + "'/>";

                break;
            case "password":
                InputType2 = "<input class='form-control' type='" + settings.secondInput + "' id='sndTxt" +
                    SmartMSGboxCount + "' placeholder='" + settings.secondPlaceholder + "'/>";
                break;

            case "select":
                if (settings.secondOptions == undefined) {
                    alert("For this type of input, the secondOptions parameter is required.");
                } else {
                    InputType2 = "<select class='form-control' id='sndTxt" + SmartMSGboxCount + "'>";
                    for (var i = 0; i <= settings.secondOptions.length - 1; i++) {
                        if (settings.secondOptions[i] == "[") {
                            Name = "";
                        } else {
                            if (settings.secondOptions[i] == "]") {
                                NumBottons = NumBottons + 1;
        ...
        if (HasInput == 1) {
            // MYDIGI support two possible inputs
            if ($("#txt" + SmartMSGboxCount).length) {
                $("#txt" + SmartMSGboxCount).focus();
            } else {
                $("#sndTxt" + SmartMSGboxCount).focus();
            }
        }
        ...
        $(".botTempo").click(function () {
            ...
            if (HasInput == 1) {
                if (typeof callback == "function") {
                    ...
                    var Value1 = null;
                    if ($("#txt" + IDNumber).length) {
                      Value1 = $("#txt" + IDNumber).val();
                    }
                    var Value2 = null;
                    if ($("#sndTxt" + IDNumber).length) {
                      Value2 = $("#sndTxt" + IDNumber).val();
                    }
                    if (callback)
                        callback(Press, Value1, Value2);
                }
            } else {
                if (typeof callback == "function") {
                    if (callback)
...
(function ($) {
    $.smallBox = function (settings, callback) {
        ...
        // MYDIGI support passing custom container
        if (SmallBoxes == 1) {
            if (settings.container == undefined) {
               $("#divSmallBoxes").append(BoxSmall);
            } else {
               settings.container.append(BoxSmall);
            }
