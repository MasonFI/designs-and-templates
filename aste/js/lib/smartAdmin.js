	...
	// MYDIGI This dom object are not ready at the time of loading this script
	// put it in a function to call later
	$.root_ = $('body');
	function setDomRef() {
...
// MYDIGI change from document ready, since we are using require js for
// lazy load. This is call separatey when view is ready.
var smartAdminReady = function() {
	setDomRef();
	setResizeCallbacks();
	/*
	 * Fire tooltips
...
// MYDIGI This resize event attachment needs to be done in the de facto
// document ready, i.e., smartAdminReady
function setResizeCallbacks() {
...
// MYDIGI change to accept specific jQuery object to speed up
// page initialization after AJAX load

function runAllForms(obj) {
	 ...
	 */
	if ($.fn.slider) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
	...
	if ($.fn.select2) {
		...
		target.each(function() {
			...
			// MYDIGI support empty string customization
			var options = {
				//showSearchInput : _showSearchInput,
				allowClear : true,
				width : width
			};
			if ($this.attr("data-no-match")) {
				var nomatch = $this.attr("data-no-match");
				options.formatNoMatches = function() {return nomatch;};
			}
	...
	if ($.fn.mask) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			var target = obj.find('[data-mask]');
		} else {
			var target = $('[data-mask]');
		}
	...
	if ($.fn.autocomplete) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			var target = obj.find('[data-autocomplete]');
		} else {
			var target = $('[data-autocomplete]');
		}
	...
	if ($.fn.datepicker) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			var target = obj.find('.datepicker');
		} else {
		...
		target.each(function() {
			...
			var dataDateFormat = $this.attr('data-dateformat') || 'd.m.yy';  // MYDIGI
	 ...
	 */
	if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
		var target = obj.find('button[data-loading-text]');
	} else {
		var target = $('button[data-loading-text]');
	}
...
function runAllCharts(obj) {
	...
	if ($.fn.sparkline) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			var target = obj.find(".sparkline");
		} else {
			var target = $('.sparkline');
		}
	...
	if ($.fn.easyPieChart) {
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			var target = obj.find(".easy-pie-chart");
		} else {
			var target = $('.easy-pie-chart');
		}
...
function setup_widgets_desktop(obj) {
	...
	if ($.fn.jarvisWidgets && $.enableJarvisWidgets) {
		...
		if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
			...
			var target = $('#widget-grid');
		}
		target.jarvisWidgets({

			grid : 'article',
			widgets : '.jarviswidget',
			// MYDIGI disable local storage, we are not gonna use local storage to keep
			// track of the sorting of widgets.
			localStorage : false,
			...
			fullscreenClass : 'fa fa-arrows-v | fa fa-arrows-v',
			fullscreenDiff : 3,
...
*/
	if ($.device === "desktop"){
		...
		// setup widgets
		setup_widgets_desktop(obj);
	} else if ($.enableMobileWidgets) {
		// setup widgets
		setup_widgets_desktop(obj);
	}
	...
	// activate popovers
	if (typeof obj !== "undefined" && obj && obj instanceof jQuery) {
		obj.find("[rel=popover]").popover();

		// activate popovers with hover states
		obj.find("[rel=popover-hover]").popover({
			trigger : "hover"
		});
	} else {
		$("[rel=popover]").popover();
...
// MYDIGI pop popover down on mousedown and include dynamic created popover marked with
// custom-popover class
$('body').on('mousedown', function(e) {
	$('[rel="popover"], .custom-popover').each(function() {
		//the 'is' for buttons that trigger popups
		//the 'has' for icons within a button that triggers a popup
		if (!$(this).is(e.target) && $(this).has(e.target).length === 0 && $('.popover').has(e.target).length === 0) {
			$(this).popover('hide');
