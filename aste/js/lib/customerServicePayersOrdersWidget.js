  ...
  function ($, jui, DP, util, tr, moment, Notification, viivaGraph, viivaGraphSettings, Ember, DialogManager, LekaJumper, underscore) {
    ...
    return function () {
      ...
      this.loadData = function (customerId) {
        ...
        DP.execute({type: "apiOrder", method: "getPayersOrders", params: data}).then(function (response) {

          if (document.getElementById("jarviswidget-fullscreen-mode")) {
            setTimeout(function () {
              $('.payers-orders-fullscreen-column').removeClass('hidden');
              $('.payers-orders-fullscreen-row').removeClass('hidden');
            }, 10);
          }
      ...
      this.populateOrderInformation = function (response) {
        ...
        $customerOrdersTable.append("<tr><th></th><th>" + tr("widgetProduct") + "</th>" +
          ...
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetOfferNumber") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetOT") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetSalesPerson") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetArchivingNumber") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetSaveDate") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetPeriodNumber") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetStart") + "</th>" +
          "<th class='payers-orders-fullscreen-column hidden'>" + tr("widgetDeliveryMethod") + "</th>" +
          "</tr>");
        ...
        if (response.message === 'Failed') {
          ...
          if (customerOrders.leka.length !== 0) {
            ...
            for (var i = 0; i < customerOrders.leka.length; i++) {
              ...
              var row = $("<tr" + (shouldHideOrderRow ? " class='payers-orders-fullscreen-row hidden'" : "  ") + " leka-order-status='" + orderStatus + "' order-start-date='" + customerOrders.leka[i].orderStartDate + "' order-end-date='" + customerOrders.leka[i].orderEndDate + "' order-type='" + customerOrders.leka[i].salesMethod + "'>" +
                ...
                "</td>" +
                "<td><a href='#' class='changeCustomer'>" + customerOrders.leka[i].receiverCustomerNumber + "</a></td>" +
                ...
                "<td class='payers-orders-fullscreen-column hidden'>" + customerOrders.leka[i].benefitCode + "</td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='campaignSubcode'>" + customerOrders.leka[i].campaignSubCode + "</span></td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='lekaCustomerJump' leka-operation='salesPerson' data='" + customerOrders.leka[i].salesPersonNumber + "'><a href='###' class='blue-underline leka-jumpper'>" + customerOrders.leka[i].salesPersonNumber + "</a></span></td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='archivingNumber'>" + customerOrders.leka[i].archivingNumber + "</span></td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='orderProcessingDate'>" + orderProcessingDate + "</span></td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='lekaCustomerJump'  leka-operation='paymentEntry' data='" + customerOrders.leka[i].orderNumber + "' data2='" + customerOrders.leka[i].invoiceInstructionNumber + "'><a href='###' class='blue-underline leka-jumpper'>" + customerOrders.leka[i].paymentEntryCount + "</a></span></td>" +
                "<td class='payers-orders-fullscreen-column hidden'><span class='orderStartNumber'>" + customerOrders.leka[i].orderStartNumber + "</span></td>" +
                "<td class='payers-orders-fullscreen-column hidden full-screen order-control-bar'><span class='deliveryType'>" + orderDeliveryType + "</span></td>" +
                "</tr>"
              );
          ...
          if (customerOrders.mydigi.length !== 0) {
            ...
            for (var j = 0; j < customerOrders.mydigi.length; j++) {
              ...
              var $myDigiPackageJ = $("#mydigipayerspackage" + j + "");
              $myDigiPackageJ.data('orderData', customerOrders.mydigi[j]);
              $myDigiPackageJ.append("<table class='table control-table table-hover table-striped' id ='mydigipayerstable" + j + "'></table>");
              ...
              if (!customerOrders.mydigi[j].orders && (myDigiOrderStatus === 'peruutettu' || myDigiOrderStatus === 'päättynyt')) {
                $myDigiPackageJ.addClass('payers-orders-fullscreen-row');
              ...
              for (var k = 0; k < customerOrders.mydigi[j].orders.length; k++) {
                ...
                $("#mydigipayerstable" + j).append("<tr" + (shouldHideOrderRow ? " class='payers-orders-fullscreen-row hidden'" : "  ") + " status=" + statusLight + " dialog-start-date=" + customerOrders.mydigi[j].orders[k].orderStartDate + " dialog-end-date=" + customerOrders.mydigi[j].orders[k].orderEndDate + " >" +
                  "<td><div class='traffic-light " + statusLight + "'>" + (statusLight === "paytrouble" ? "<btn class='fa fa-ban' style='color: red; font-size: 1.5em;''></btn>" : " ") + "</div>" +
                  "<div class='cancellation'>" +
                  ...
                  "</td>" +
                  "<td><a href='#' class='changeCustomer'>" + customerOrders.mydigi[j].orders[k].receiverCustomerNumber + "</a></td>" +
                  "<td>" + orderStartDate + " - " + orderEndDate + "</td>" +
                  "<td>" + orderSalesMethod + "</td>" +
                  ...
                  "<td class='payers-orders-fullscreen-column hidden'>" + customerOrders.mydigi[j].orders[k].benefitCode + "</td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='campaignSubcode'>" + customerOrders.mydigi[j].orders[k].campaignSubCode + "</span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='lekaCustomerJump' leka-operation='salesPerson' data='" + customerOrders.mydigi[j].orders[k].salesPersonNumber + "'><a href='###' class='blue-underline leka-jumpper'>" + customerOrders.mydigi[j].orders[k].salesPersonNumber + "</a></span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='archivingNumber'>" + customerOrders.mydigi[j].orders[k].archivingNumber + "</span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='orderProcessingDate'>" + orderProcessingDate + "</span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='lekaCustomerJump'  leka-operation='paymentEntry' data='" + customerOrders.mydigi[j].orders[k].orderNumber + "' data2='" + customerOrders.mydigi[j].orders[k].invoiceInstructionNumber + "'><a href='###' class='blue-underline leka-jumpper'>" + customerOrders.mydigi[j].orders[k].paymentEntryCount + "</a></span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden'><span class='orderStartNumber'>" + customerOrders.mydigi[j].orders[k].orderStartNumber + "</span></td>" +
                  "<td class='payers-orders-fullscreen-column hidden full-screen order-control-bar'><span class='deliveryType'>" + orderDeliveryType + "</span></td>" +
                  "</tr>");

                if (k === customerOrders.mydigi[j].orders.length - 1 && !myDigiOrderHasVisibleRows) {
                  $("#mydigipayerspackage" + j + "").addClass('payers-orders-fullscreen-row hidden');
                }
              }
            }
          }

          var $ordersWidget = $('#payersOrdersWidget');
          ...
          // cancellation bar
          $customerOrdersTable.find('td.order-control-bar').append(
            "<div class='controls'>" +
          ...
          // LEKA ONLY cancels
          $('.control-button .cancel-order').click(function (e) {
            e.preventDefault();
      ...
      this.bindEvents = function () {
        ...
        $('#payersOrdersWidget').on('click', 'a.changeCustomer', function (event) {
          event.preventDefault();
          App.Router.router.transitionTo('customerService', $(this).text());
        });

        // new order click
        var $orders = $('#payers-orders');
        ...
        $orders.on('click', function (event) {
          if (document.getElementById("jarviswidget-fullscreen-mode")) {
            ...
            $('.payers-orders-fullscreen-column').addClass('hidden');
            $('.payers-orders-fullscreen-row').addClass('hidden');
      ...
      this.create = function (insideElement) {
        ...
        this.view = $("<div class='jarviswidget jarviswidget-color-blueDark' data-widget-deletebutton='false' " +
          "data-widget-colorbutton='false' data-widget-editbutton='false' data-widget-togglebutton='false' id='payers-orders'>" +
          "<header>" +
          "<h2>Maksajana tilauksille</h2> " +
          "</header>" +
          "<!-- widget div-->" +
          "<div class='widget-body' id='payersOrdersWidget'>" +
          "<!-- widget edit box -->" +
          "<table class='table control-table table-striped table-hover' style='padding-left: 4px' id='customer-payers-orders-table'>" +
          "<thead>" +
          "</thead>" +
          "<tbody>" +
          "</tbody>" +
          "</table><br><br>" +
          "<div class='mydigi-payers-order-area'></div>" +
